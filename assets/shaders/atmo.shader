...
//#define ATMO_FULL
//#define HORIZON_HACK
render_mode cull_disabled, unshaded, depth_test_disable, blend_add;
...
uniform vec4 mixColor:hint_color = vec4(0);
...
vec2 ray_sphere_intersect(
    ...
    vec3 viewDir, // the direction of the ray
...
//			else
//			{
				ALBEDO = (color.rgb * extinction2  + inscatter2 );
//				ALBEDO = (color.rgb + mixColor.xyz * (dis / maxDistance) * scaleFactor);
//				ALBEDO = (finalColor);
				ALPHA = alpha;
//			}
//			ALBEDO = ( inscatter2);
...
//	float factor = length(normalize(scene));
//	factor = tanh(factor);
//    vec3 finalColor = mix(sunColor * extinction + inscatter, scene, factor);
//    ALBEDO = vec3(1, 0, 0);
}
