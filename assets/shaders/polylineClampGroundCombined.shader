...
void getAttributes(int startIndex, int batchId, out vec3 start, out vec3 forward, out vec3 startNormal, out vec3 endNormal, out vec3 rightPlaneNormal)
{
	startIndex = startIndex + batchId * 15;

	start.x = RGBA2Float(texelFetch(vertexs_array, ivec2(startIndex / 512, startIndex % 512), 0));
	start.y = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 1) / 512, (startIndex + 1) % 512), 0));
	start.z = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 2) / 512, (startIndex + 2) % 512), 0));
	
	forward.x = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 3) / 512, (startIndex + 3) % 512), 0));
	forward.y = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 4) / 512, (startIndex + 4) % 512), 0));
	forward.z = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 5) / 512, (startIndex + 5) % 512), 0));
	
	startNormal.x = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 6) / 512, (startIndex + 6) % 512), 0));
	startNormal.y = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 7) / 512, (startIndex + 7) % 512), 0));
	startNormal.z = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 8) / 512, (startIndex + 8) % 512), 0));
	
	endNormal.x = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 9) / 512, (startIndex + 9) % 512), 0));
	endNormal.y = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 10) / 512, (startIndex + 10) % 512), 0));
	endNormal.z = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 11) / 512, (startIndex + 11) % 512), 0));
	
	rightPlaneNormal.x = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 12) / 512, (startIndex + 12) % 512), 0));
	rightPlaneNormal.y = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 13) / 512, (startIndex + 13) % 512), 0));
	rightPlaneNormal.z = RGBA2Float(texelFetch(vertexs_array, ivec2((startIndex + 14) / 512, (startIndex + 14) % 512), 0));
}
...
void vertex()
{
	...
	VERTEX += width / UV2.y / 2.0  * NORMAL * metersPerPixel;
	halfMaxWidth = width / 2.0;
	vec3 start;
	vec3 offset;
	vec3 startPlaneNormal; 
	vec3 endPlaneNormal;
	vec3 rightPlaneNormal;
	
	getAttributes(int((UV.x)),int(floor(UV2.x)), start, offset, startPlaneNormal, endPlaneNormal, rightPlaneNormal);
...
//		ALPHA = 0.11;
	...
	// if(abs(disToRightPlane) / (halfMaxWidth * metersPerPixel) > 0.8)
	// {
	// 	ALBEDO = vec3(0,1,0);
	// }
	// else
